...
# Get the top magnitudes
new_nodes = sorted(nodes, key = lambda x: x.magnitude(), reverse = True)

max_mag = 0
for i in range(len(new_nodes)):
    for o in range(len(new_nodes)):
        if i == o:
            continue
        
        node_1 = new_nodes[i].copy()
        node_2 = new_nodes[o].copy()
        mag = (node_1 + node_2).magnitude()
